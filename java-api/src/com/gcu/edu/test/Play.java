	...
	private static boolean listAllSwarmServices()
	{
		...
		servicesInfo.forEach(serviceInfo -> System.out.println(String.format("=======> Service with a ID of %s\n"
																		   ...
																		   + "         with Ports of %s=>%s\n",
																						...
																						serviceInfo.getSpec().getTaskTemplate().getResources().getLimits().getNanoCPUs() == null ? -1: serviceInfo.getSpec().getTaskTemplate().getResources().getLimits().getNanoCPUs().floatValue() / 1000000000f,
																						serviceInfo.getSpec().getTaskTemplate().getResources().getLimits().getMemoryBytes() == null ? -1: serviceInfo.getSpec().getTaskTemplate().getResources().getLimits().getMemoryBytes() / 1000000,
