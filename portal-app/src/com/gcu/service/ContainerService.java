...
public class ContainerService implements ContainerServiceInterface 
{
	...
	{
		// Validate the Image being called for exists. Exceptions handled in imageService
		imageService.findImageById(container.getImageId());
		 ...
		 /* TODO: Get the Docker Swarm Generated DockerID. Current DockerID is randomly generated

			
		 // Request API to create the container and return the DockerID
		 String dockerId = API.createSwarmService(container);
		 
		 // If service did not return an ID, throw exception and exit method
		 if(dockerId == null)
		 {
		 	throw new DockerSwarmException();
		 }
		  */
	...
	{
		// Call Container DAO to assemble a list of all containers associated to the username
	...
	public boolean deleteContainer(Container container, User user) throws ContainerNotFoundException 
	{
		...
		/* TODO: Request the API to delete the Container from the Swarm
		 ...
		 // Request API to delete the container
		 boolean success = API.deleteSwarmService(container.dockerId);
		 
		 // If the API did not delete the container from Swarm, throw exception
		 if (success == false)
		 {
		 	throw new DockerSwarmException();
		 }
